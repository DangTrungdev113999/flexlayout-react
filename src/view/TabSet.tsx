...
export const TabSet = (props: ITabSetProps) => {
    ...
    const { position, userControlledLeft, hiddenTabs, onMouseWheel, tabsTruncated } = useTabOverflow(node, Orientation.HORZ, toolbarRef, stickyButtonsRef);
    ...
    const onMouseDown = (event: React.MouseEvent<HTMLDivElement, MouseEvent> | React.TouchEvent<HTMLDivElement>, isDragging: boolean = false) => {
    ...
    const toolbar = (
        <div key="toolbar" ref={toolbarRef} className={cm(CLASSES.FLEXLAYOUT__TAB_TOOLBAR)} onMouseDown={(e) => onMouseDown(e, true)} onTouchStart={(e) => onMouseDown(e, true)}>
            {buttons}
        </div>
    );
    ...
    return (
        <div dir="ltr" data-layout-path={path} style={style} className={cm(CLASSES.FLEXLAYOUT__TABSET)} onWheel={onMouseWheel}>
            {content}
        </div>
    );
};
